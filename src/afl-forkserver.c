...
void afl_fsrv_init(afl_forkserver_t *fsrv) {
  ...
  fsrv->sym_trace_fd = -1;
  fsrv->sym_trace_buf = NULL;
  fsrv->sym_trace_len = 0;
  fsrv->sym_trace_cap = 0;
...
fsrv_run_result_t __attribute__((hot)) afl_fsrv_run_target(
  ...
  // The buffer belongs to one target execution. It is consumed by the custom
  // mutator before AFL starts the next run.
  fsrv->sym_trace_len = 0;
  ...
  exec_ms = fsrv->sym_trace_fd >= 0
                ? read_s32_timed_with_trace(fsrv, &fsrv->child_status, timeout,
                                             stop_soon_p)
                : read_s32_timed(fsrv->fsrv_st_fd, &fsrv->child_status, timeout,
                                 stop_soon_p);
...
void afl_fsrv_deinit(afl_forkserver_t *fsrv) {
  ...
  afl_fsrv_kill(fsrv);
  if (fsrv->sym_trace_buf) {

    ck_free(fsrv->sym_trace_buf);
    fsrv->sym_trace_buf = NULL;
    fsrv->sym_trace_len = fsrv->sym_trace_cap = 0;
